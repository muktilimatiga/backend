...
def build_cek_action_menu() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Reboot Modem", callback_data="cek_action:reboot")],
        [InlineKeyboardButton("📊 Port Status", callback_data="cek_action:port_state")],
        [InlineKeyboardButton("📶 Port Redaman", callback_data="cek_action:port_rx")],
...
async def handle_olt_selection(query, session: UserSession, data: str):
    ...
    await query.edit_message_text(f"⏳ Mendeteksi ONT di *{olt_name}*...", parse_mode="Markdown")
    ...
    try:
        ...
        if not ont_list:
            await query.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 Refresh", callback_data=f"olt_{olt_name}")],
    ...
    try:
        ...
        if not psb_list:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔄 Refresh PSB", callback_data="refresh_psb")],
...
async def run_cek_onu(query, session: UserSession):
    ...
    try:
        ...
        await query.edit_message_text(
            f"❌ Gagal cek ONU:\n{str(e)[:500]}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔃 Retry", callback_data="cek_action:refresh")],
                [InlineKeyboardButton("❌ Exit", callback_data="cek_action:exit")]
            ])
        )
    finally:
    ...
    if action == "exit":
        ...
        if session.last_action == "port_state":
            ...
            try:
                ...
                await query.edit_message_text(f"❌ Error: {str(e)[:200]}", reply_markup=build_cek_action_menu())
            finally:
                session.is_busy = False
    ...
    try:
        ...
        if len(customers) == 1:
            ...
            if is_callback:
                ...
                msg = await update_or_query.message.reply_text(f"⏳ Mengecek ONU untuk {name}...")
